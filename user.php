...
class user extends dbh
{
	...
	public function login($username, $password)
	{
		...
		if($username == 'admin@admin.com' && $password == 'pass3word')
		{
			...
			if ($numberrows > 0 ) 
			{
				...
				{
					$error = 3;
					echo '<div class ="alert alert-danger">'.$this->messages($error).'</div>';
				}	
	...
	public function checkPatient($phoneNo)
	{
		if (!empty($phoneNo)) {
			# code...
			$stmt = "SELECT * FROM patient where phoneNo = '$phoneNo' ";
			$result = $this->connect()->query($stmt);
			$numberrows = $result->num_rows; 
			if (($numberrows)> 0) {
			...
			return '<div class ="alert alert-danger alert-dismissible"> <strong> All fields are required </strong> </div>';
		}
		
	}

	// Insert Pateint
	public function insertpatient($fullname,$dob,$hospID,$gender,$phoneNo){
		if (empty($this->checkPatient($phoneNo))) 
		{
			$date = date('Y-m-d');
			$insert = "INSERT INTO patient(name,dob,hospitalID,sex,phoneNo,created_at)Values('$fullname','$dob','$hospID','$gender','$phoneNo','$date')";
			$stmt = $this->connect()->query($insert);
			if ($stmt) {
				echo '<div class ="alert alert-success alert-dismissible"> <strong> New Pateint Record  Added Successfully </strong> </div>';
			}
			else
			{
			...
			echo $this->checkPatient($phoneNo);
		}
	...
	// Update Pateint
	public function update_patient($fullname,$sex,$phone,$id){

	 $stmt = "UPDATE patient set sex = '$sex', phoneNo = '$phone', name='$fullname' where id = '$id'";
	 $result = $this->connect()->query($stmt);
	 if($result)
	 {
	 	echo "success";
	...
	{
		$stmt = "DELETE FROM patient where id = '$id'";
		$result = $this->connect()->query($stmt);
		if ($result) {
				echo "success";
			}
...
// Get Vaccine Names based by ID
	...
	public function getChildrenInHW($id)
	{
		$stmt = "SELECT * FROM children where health_facility_id = '$id'";
		$result = $this->connect()->query($stmt);
		$numberrows = $result->num_rows;
		return $numberrows;
